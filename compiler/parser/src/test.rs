...
/// id and span fields just clutter up the output and
/// may produce spurious diffs.
fn remove_id_and_span(value: &mut serde_json::Value) {
    use serde_json::Value::*;

    match value {
        Array(array) => array.iter_mut().for_each(remove_id_and_span),
        Object(map) => {
            map.remove_entry("id");
            map.remove_entry("span");
            map.values_mut().for_each(remove_id_and_span);
        }
        _ => {}
    }
}
...
fn run_parse_many_test<T: Serialize>(
    ...
    let mut value = serde_json::to_value(&serializable).expect("Serialization failure");
    remove_id_and_span(&mut value);
...
fn run_parser_test(test: &str, handler: &Handler) -> Result<String, ()> {
    ...
    let mut value = serde_json::to_value(&ast.ast).expect("Serialization failure");
    remove_id_and_span(&mut value);
    let mut s = serde_json::to_string_pretty(&value).expect("string conversion failure");
    s.push('\n');
    Ok(s)
